...
TEST(Csv_ReaderTest, read) {
    ...
    EXPECT_EQ(csv_reader.vec_mem_util_percent[0], std::string("99"));
}
...
TEST(Csv_ReaderTest, write) {
    ...
    std::string time_stamp, cpu_util_percent, mem_util_percent;
    infile >> time_stamp >> cpu_util_percent >> mem_util_percent;
    infile.close();
    EXPECT_EQ(time_stamp, std::string("220740,220750"));
    EXPECT_EQ(cpu_util_percent, std::string("20,20"));
    EXPECT_EQ(mem_util_percent, std::string("99,99"));
...
TEST(LoserTreeTest, merge_sort) {
    ...
    std::vector<std::vector<std::string>> cpu_util_percents = {
            ...
            {"60", "90",  "100"},
            {"70", "110", "120"}};
    std::vector<std::vector<std::string>> mem_util_percents = {
            {"10", "30",  "50"},
            {"20", "40",  "80"},
            {"60", "90",  "100"},
            {"70", "110", "120"}};
    LoserTree lt(time_stamps, cpu_util_percents, mem_util_percents);
    ...
    for (int i = 0; i < vec.size(); ++i) {
        std::string expect_str =
                std::to_string(i + 1) + "," + std::to_string((i + 1) * 10) + "," + std::to_string((i + 1) * 10);
...
TEST(ResortTest, sort_one_file) {
    ...
    for (int i = 1; i <= 15; ++i) {
        ...
        std::string mem_util_percent = std::to_string(i * 10);
        EXPECT_EQ(line, time_stamp + "," + cpu_util_percent + "," + mem_util_percent);
    }
    infile.close();
}
...
TEST(ResortTest, sort_files) {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        for (int j = 1; j <= 15; ++j) {
            ...
            std::string mem_util_percent = std::to_string(j * 10);
            EXPECT_EQ(line, time_stamp + "," + cpu_util_percent + "," + mem_util_percent);
        }
        infile.close();
    }
