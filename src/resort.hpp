...
public:
    ...
    LoserTree(std::vector<std::vector<std::string>> &time_stamps,
              std::vector<std::vector<std::string>> &cpu_util_percents,
              std::vector<std::vector<std::string>> &mem_util_percents)
            : time_stamps(time_stamps), cpu_util_percents(cpu_util_percents), mem_util_percents(mem_util_percents) {
        n = time_stamps.size();
        for (int i = 0; i < n; ++i)
    ...
    void merge_sort(std::vector<std::string> &result) {
        ...
        while (leaves[i] != max_k) {
            ...
            mem_util_percent = mem_util_percents[i][index[i] - 1];
            intput(i);
            adjust(i);
            i = loser_tree[0];
            result.push_back(time_stamp + "," + cpu_util_percent + "," + mem_util_percent);
        }
...
public:
    ...
    void sort_one_file(int container_id) {
        ...
        while (infile) {
            ...
            std::getline(infile, line);
            mem_util_percents.push_back(split_str(line));
        }

        // sort
        std::vector<std::string> result;
        LoserTree loset(time_stamps, cpu_util_percents, mem_util_percents);
    ...
    void output(int total) {
        ...
        for (auto it = container_part_data_missing.begin();
            ...
            out2 << it->first << "," << int(it->second * 100) << "\n";
        }
        out2.close();
